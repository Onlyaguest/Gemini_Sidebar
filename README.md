...
- 「辅助小工具」分组：参考文档 / 对话导出 / 批量图片 / 自由框（默认收起）
- 自由框支持临时记录与“固化”保存，一键插入输入框
- 一键导出当前对话为 Markdown（全部消息 / 仅我方提问）
- 批量下载对话里的所有图片（支持选择 + 并行极速模式）
...
8. 在「辅助小工具」中进行参考文档导入 / 对话导出 / 批量图片 / 自由框使用
...
- 入口：辅助小工具 → 批量图片 → 扫描并选择下载
- 默认模式：模拟点击官方“下载图片”按钮，文件名遵循 Gemini 官方命名
- 极速模式：使用直链并行下载，速度更快，文件名以 `gemini-img` 前缀生成
...
- 图片下载为空：先滚动页面，确保对话中的图片已加载完成。
- 图片下载失败：检查浏览器是否允许多文件下载，或关闭极速模式重试。
